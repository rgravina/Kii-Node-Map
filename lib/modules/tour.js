...
module.exports = function () {
  ...
  var gauge = grid.set(0, 9, 2, 3, contrib.gauge, {label: 'Creating Objects', stroke: 'magenta', fill: 'magenta'});
  var box = grid.set(2, 9, 10, 3, blessed.box, {content: 'Search'})
  ...
  // store countries if not already there
  var countriesBucket = Kii.bucketWithName("countries");
  var point, object;
  var length = _.keys(countries).length;
  index = 0;
  _.each(countries, function(country) {
    point = KiiGeoPoint.geoPoint(country["lat"], country["lon"]);
    object = countriesBucket.createObject();
