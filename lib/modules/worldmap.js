...
var worldMap = {
  ...
  login: function() {
    return new Promise(function (resolve, reject) {
      ...
      if (user == null) {
        worldMap.log("Attempting to log in 'worldmap' user...");
        KiiUser.authenticate("worldmap", "worldmap", {
          success: function(theUser) {
            worldMap.log("User 'worldmap' found.");
            worldMap.user = theUser;
            resolve(theUser);
          },
          failure: function(theUser, errorString) {
            worldMap.log("User 'worldmap' not found. Attempting to register 'worldmap' user...");
            user = KiiUser.userWithUsername("worldmap", "worldmap");
            user.register({
              success: function(theUser) {
                worldMap.log("User 'worldmap' registered.");
                worldMap.user = user;
                resolve(user);
              },
              failure: function(theUser, errorString) {
  ...
  showSearchForm: function() {
    ...
    this.resetBtn.on('press', function() {
      worldMap.form.reset();
      _.each(worldMap.checkboxes, function(checkbox) {
  ...
  search: function(data) {
    ...
    */
    ...
    queryCallbacks = {
      success: function(queryPerformed, resultSet, nextQuery) {
        ...
        if(nextQuery != null) {
          ...
          countriesBucket.executeQuery(nextQuery, queryCallbacks);
        } else {
    ...
    countriesBucket.executeQuery(query, queryCallbacks);
  },
...
module.exports = function () {
  ...
  var user = worldMap.login().then(function(user) {
    ...
    worldMap.log("Creating empty object...");
    ...
    emptyObject.saveAllFields({
      success: function(savedObj) {
        worldMap.log("Empty object created.");
        var point, object;
        var length = _.keys(countries).length;
        index = 0;
        worldMap.log("Creating countries...");
          ...
          object.saveAllFields({
            success: function(savedObj) {
              ...
              if (percent == 100) {
                emptyObject.delete({
                  success: function() {
                    worldMap.log("Deleted empty object.");                    
            ...
            failure: function(savedObj, error) {
              ...
              worldMap.log(error);
              // increment index anyway so that it completes
              index++;
            }
          });
          worldMap.render();
